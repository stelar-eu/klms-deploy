// Core Deployment constructor for the previewer component.
...

{
  new(config):
    deploy.new(
      ...
      containers = [
        container.new(config.deployment.container_name, config.images.PREVIEWER_IMAGE)
        ...
        + container.withPorts([
          containerPort.newNamed(config.ports.PREVIEWER, config.service.port_name),
        ]),
      ],

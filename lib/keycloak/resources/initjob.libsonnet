...
local env(config) = {
  ...
  KC_QUAY_PULLERS: if std.objectHas(config, "quay") then config.quay.QUAY_PULLERS_ROLE else "",
  KC_QUAY_PUSHERS: if std.objectHas(config, "quay") then config.quay.QUAY_PUSHERS_ROLE else "",

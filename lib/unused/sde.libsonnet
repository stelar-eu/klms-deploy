...
local PORT = import "../util/stdports.libsonnet";
...

{
    manifest(pim,config): {
        ...
        flink_cluster: deploy.new(name="flink-cluster", containers=[
              container.new("jmanager", 'petroud/stelar-tuc:sde')
            + container.withImagePullPolicy("Always")
            + container.withEnvMap({
            ...
            + container.withPorts([
                containerPort.newNamed(6123, "fl-rpc"),
                containerPort.newNamed(8081, "fl"),
            ])
            + container.withArgs(['jobmanager']),

              container.new("tmanager1", "petroud/stelar-tuc:sde")
            + container.withImagePullPolicy("Always")
            + container.withEnvMap({
                JOB_MANAGER_RPC_ADDRESS: "localhost",
            })
            + container.withPorts([
                containerPort.newNamed(6121,"fl-t2"),
            ])
            + container.withArgs(['taskmanager']),

             container.new("tmanager2", "petroud/stelar-tuc:sde")
            + container.withImagePullPolicy("Always")
            + container.withEnvMap({
                JOB_MANAGER_RPC_ADDRESS: "localhost",
            })
            + container.withPorts([
                containerPort.newNamed(6122,"fl-t1"),
            ])
             + container.withArgs(['taskmanager']),
        ...
        flink_svc: svcs.serviceFor(self.flink_cluster),

...
• Findings
  ...
  - Resolved: namespace is environment-owned. `lake create --minimal --namespace` writes
    `spec.namespace`, product/fullspec files do not contain `klms.namespace`, and Jsonnet
    rendering receives namespace through `config.environment.namespace`.

  - Resolved: bootstrap state now records a product fullspec hash. `lake activate`,
    `lake bootstrap`, and `lake verify` refuse to act when the selected fullspec no
    longer matches the recorded bootstrapped product. `lake status` remains diagnostic:
    it reports recorded bootstrap Secret state, inspects current active-product workloads
    when possible, and prints a bad-state diagnostic with the `lake purge-secrets` recovery
    path instead of silently reporting product A as product B.

  - Resolved: lake remove refuses environments that contain an active product or
    recorded bootstrap state unless `--force` is supplied. This preserves the local
    metadata needed by `tk delete ENV` and `lake purge-secrets ENV` during normal cleanup.

  - Resolved: lake add --adopt-existing-main now validates that an existing
    main.jsonnet matches the managed stelarctl entrypoint template before marking
    the directory as a lake environment. Custom render paths are rejected.

  - Resolved: lake create now warns when regenerating the currently active product
    name leaves spec.stelar.active_product pointing at the previous fullspec. Before
    bootstrap, the user is told to run lake activate; after bootstrap, the user is
    told to purge old bootstrap Secrets, activate the regenerated product, and
    bootstrap the regenerated fullspec.

  - Resolved: lake create --minimal --context writes spec.contextNames even when
    --infer-storage-from-cluster is not used; this behavior is covered by tests.

  - Resolved: lake status --wait requires a positive CLI --poll-interval value,
    while operation-level tests can still pass zero internally.

...
from stelar.deploy.flib.feature import Feature, FeatureModel, SubfeatureGroup
...
def check_parent_and_model(feature: Feature, parent: Feature | None, fmodel: FeatureModel):
    assert feature.parent == parent, f"Feature parent error: {feature}"
    assert feature.fmodel == fmodel, f"Feature {feature.name} has fmodel {feature.fmodel.name if feature.fmodel else None}, expected {fmodel.name if fmodel else None}"
